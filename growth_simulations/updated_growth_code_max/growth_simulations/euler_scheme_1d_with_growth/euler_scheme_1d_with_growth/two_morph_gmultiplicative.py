...
class two_morph_euler:
    ...
    def run_simulation(self):
        ...
        for n in tqdm(range(1, self.ndt)):
            if growth_has_stopped == False:
                ...
                # instead stop growth, if growth region is down to less than 2 bins in size
                if np.sum(growth_region) < 2:
                    growth_region = 0 * growth_region
                    growth_has_stopped = True

                x_grid_prev_gr = x_grid_prev * (1 + self.g * (s_prev-self.threshold_growth) * (f_prev-self.threshold_growth)  * self.dt * growth_region )
                ...
                s_prev = refl_bound(s_prev * (x_grid_prev / x_grid_prev_gr))
                f_prev = refl_bound(f_prev * (x_grid_prev / x_grid_prev_gr))

                sn = self.D * laplacian_non_unif_grid(s_prev, x_centers_prev_gr) * self.dt + s_prev 
                fn = self.D * laplacian_non_unif_grid(f_prev, x_centers_prev_gr) * self.dt + f_prev 
                
                sn += self.dt * self.source_s(x_centers_prev_gr, self.x0, self.w, L)- self.beta * s_prev * self.dt
                fn += self.dt * self.source_f(x_centers_prev_gr, self.x0, self.w, L)- self.beta * f_prev * self.dt
    ...
    def save_results(self, folder_base='growth_simulations/figure_s5_effect_of_growth/results_of_numerics/two_morph_gmultiplicative/dynamics/'):
        # Create folder path
